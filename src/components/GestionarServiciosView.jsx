...
export default function GestionarServiciosView({ servicios = [], currentUserPermissions = [], currentUserRoleId = null, currentUserAreaId = null, onChangeAprobacion }) {
  ...
  const baseFilteredServicios = useMemo(() => {
    const searchTerm = normalizeSearch(textQuery)
    const fromTime = dateFrom ? new Date(`${dateFrom}T00:00:00`).getTime() : null
    const toTime = dateTo ? new Date(`${dateTo}T23:59:59.999`).getTime() : null

    return visibleServicios.filter((servicio) => {
      if (activePriority !== 'TODAS' && normalize(servicio.prioridad) !== activePriority) return false
      ...
      if (fromTime || toTime) {
        const servTime = new Date(servicio.fecha || 0).getTime()
        if (fromTime && (!Number.isFinite(servTime) || servTime < fromTime)) return false
        if (toTime && (!Number.isFinite(servTime) || servTime > toTime)) return false
      }

      return true
    })
  }, [visibleServicios, activePriority, textQuery, dateFrom, dateTo])
        ...
        <div className="purchase-filters-grid">
          <label className="purchase-filter-field">
            ...
            <input
              ...
              placeholder="Area, sub area, nombre, descripcion..."
            />
          </label>
