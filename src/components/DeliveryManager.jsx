  ...
  export default function DeliveryManager({
    ...
    const [ratingMode, setRatingMode] = useState('optional')

    const openRatingModal = (compra, mode = 'optional') => {
    ...
    const submitRating = async (event) => {
      ...
      try {
        if (ratingMode === 'recepcion') {
          if (onConfirmarRecepcion) {
            await onConfirmarRecepcion(ratingCompra.id, {
              puntuacion: Number(ratingForm.puntuacion || 0),
              comentario: String(ratingForm.comentario || '').trim(),
            })
          }
          setRatingNotice('Recepcion y calificacion confirmadas')
    ...
    const handleRecepcionCompra = async (compra) => {
      if (!compra.id_proveedor) {
        setError('Esta compra no tiene proveedor asignado, no se puede calificar')
        return
      }
      openRatingModal(compra, 'recepcion')
    }
          ...
          <div className="provider-modal-backdrop" onClick={closeRatingModal}>
            <div className="provider-modal provider-rating-modal" onClick={(event) => event.stopPropagation()}>
              <div className="provider-modal-head">
                <h2>{ratingMode === 'recepcion' ? 'Calificar proveedor (obligatorio)' : 'Calificar proveedor'}</h2>
                {ratingMode !== 'recepcion' && <button type="button" onClick={closeRatingModal} disabled={ratingSaving}>×</button>}
              </div>
              <div className="provider-rating-header">
                <h3>{ratingCompra.proveedor || 'Proveedor'}</h3>
                <p>{ratingMode === 'recepcion' ? 'Debes calificar al proveedor para confirmar la recepcion.' : 'Completa una calificacion breve de la entrega.'}</p>
              </div>
              ...
              <form className="provider-rating-form" onSubmit={submitRating}>
                ...
                <div className="provider-modal-actions">
                  {ratingMode !== 'recepcion' && <button type="button" onClick={closeRatingModal} disabled={ratingSaving}>Omitir</button>}
                  <button type="submit" disabled={ratingSaving}>{ratingSaving ? (ratingMode === 'recepcion' ? 'Confirmando...' : 'Guardando...') : (ratingMode === 'recepcion' ? 'Calificar y confirmar recepcion' : 'Guardar calificacion')}</button>
                </div>
              </form>
            </div>
          </div>
