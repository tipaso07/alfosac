      ...
      `
        WITH movimientos_base AS (
          SELECT
            ...
            COALESCE(m.fecha_movimiento, m.created_at) AT TIME ZONE 'America/Lima' AS fecha,
            COALESCE(
              NULLIF(to_jsonb(m)->>'usuario_registro', ''),
              NULLIF(to_jsonb(m)->>'id_usuario', ''),
              NULLIF(to_jsonb(m)->>'usuario_id', ''),
...
app.get('/api/mis-compras', authMiddleware, async (req, res) => {
  try {
    ...
    if (canAccessPurchaseOrdersModule(req.user)) {
      ...
      // Do not attach `gestion_estado_usuario` here; return canonical `estado` and `estado_pedido`.
    ...
    if (approvalStageStates.length > 0) {
      ...
      // Do not attach `gestion_estado_usuario` here; frontend can derive display state.
    ...
    if (isApprovalHierarchyRoleId(roleId) && canApproveInCurrentStage) {
      ...
      // Do not attach `gestion_estado_usuario` here; return the raw `estado` field.
        ...
        `
          SELECT
            ...
            (
              SELECT COUNT(*)
              FROM compras c
              WHERE upper(trim(COALESCE(NULLIF(to_jsonb(c)->>'estado_pedido', ''), NULLIF(to_jsonb(c)->>'estado', '')))) = 'PENDIENTE'
            ...
            ) AS total_compras_pendientes,
            (
              SELECT COUNT(*)
              FROM compras c
              WHERE upper(trim(COALESCE(NULLIF(to_jsonb(c)->>'estado_pedido', ''), NULLIF(to_jsonb(c)->>'estado', '')))) IN ('APROBADA','APROBADO')
              ${areaIds && areaIds.length > 0 ? `AND COALESCE(NULLIF(to_jsonb(c)->>'id_area_final', '')::int, NULLIF(to_jsonb(c)->>'id_area_solicitante', '')::int) = ANY($1::int[])` : ''}
            ) AS total_compras_aprobadas,
            (
              SELECT COUNT(*)
              FROM compras c
        ...
        `
          ...
          , detalle_salida AS (
            SELECT
              ...
              NULLIF(COALESCE(to_jsonb(mat)->>'moneda_id', to_jsonb(mat)->>'id_moneda', ''), '')::int AS moneda_id,
              COALESCE(mo.nombre, 'N/D') AS moneda,
            ...
            LEFT JOIN materiales mat ON mat.id = md.id_material
            LEFT JOIN monedas mo ON mo.id = NULLIF(COALESCE(to_jsonb(mat)->>'moneda_id', to_jsonb(mat)->>'id_moneda', ''), '')::int
          ),
          materiales_rank AS (
            SELECT
              material,
